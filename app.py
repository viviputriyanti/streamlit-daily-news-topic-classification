...
def preprocess_text(text):
    """Membersihkan teks input untuk prediksi."""
    text = str(text).lower()
    text = re.sub(r'\[.*?\]', '', text)
...
# --- FUNGSI SCRAPING YANG DISEMPURNAKAN (DARI REFERENSI ANDA) ---
def scrape_kontan_article(url):
    """Mengambil teks artikel dari URL kontan.co.id dengan beberapa kandidat selector."""
    try:
        ...
        # Daftar kandidat selector CSS untuk menemukan konten utama
        candidates = [
            "div.detail-konten", "div.article-content", "div.contents", 
            "div.konten-artikel", "div#content", "div.main-content", "article", "main"
        ]
        ...
        for selector in candidates:
            ...
            if container:
                ...
                # Heuristik sederhana: jika teks cukup panjang, anggap itu konten yang benar
                if len(text) > 200:
                    return text

        # Fallback jika tidak ada kandidat yang cocok: gabungkan semua paragraf
...
def display_prediction_result(text_input, model, vec):
    """Fungsi untuk memproses dan menampilkan hasil prediksi yang rapi."""
...
# --- PEMUATAN DATA DAN MODEL DI AWAL ---
...
# --- HALAMAN UTAMA (TAMPILAN LAMA ANDA) ---
...
with tab1:
    if not df_filtered.empty:
        try:
            total_artikel = len(df_filtered)
            jumlah_topik = df_filtered['label_topic'].nunique()
            ...
            with col_chart1:
                ...
                fig_bar = px.bar(topic_counts, x='count', y='label_topic', orientation='h', title=f"Distribusi Artikel")
            ...
            with col_chart2:
                ...
                fig_pie = px.pie(pie_counts, names='label_topic', values='count', title=f"Persentase Topik")
...
# --- KONTEN TAB 2: KLASIFIKASI ARTIKEL (DENGAN FUNGSI BARU) ---
with tab2:
    ...
    if ml_model and vectorizer:
        with input_manual:
            ...
            def clear_text_manual():
                st.session_state.artikel_input_manual = ""
            ...
            if predict_button_manual and user_input_manual:
                display_prediction_result(user_input_manual, ml_model, vectorizer)
            ...
            if st.button("Scrape & Prediksi", type="primary"):
                if url_input:
                    ...
                    if article_text:
                        ...
                        display_prediction_result(article_text, ml_model, vectorizer)
                    else:
